...
**Источники конкурсных списков (реестр `packages/parsers/sources.py`):**
ИТМО, ВШЭ (pilot), МФТИ (monitor) + 8 вузов через универсальный ингейшн:
МГУ, МЭИ, РНИМУ, МИСИС, РУДН, МГТУ СТАНКИН, Финансовый университет,
Сеченовский университет. Подробности — в `docs/integration-plan.md`.

**Известные ограничения (v0.1):** ВШЭ discovery API временно отдаёт 500
(адаптер в работе); МИФИ/МГСУ/МАДИ/ГУУ не подключены (списки закрыты или
отсутствуют); ФинУниверситет — один snapshot на вуз; Сеченовка — только
состав списка (без баллов). Прогноз `probabilistic-2` активен только для
ИТМО; новые вузы подключаются после накопления истории и валидации.
